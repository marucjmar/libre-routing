...
const coordEquals = (c1 = [], c2 = []) =>
  floatEquals(c1[0], c2[0]) && floatEquals(c1[1], c2[1]);
...
function distinctSegment(coordinates, coordCounts) {
  ...
  return lineString(
    longestDistinctSegment.length === 0 ? coordinates : longestDistinctSegment
  );
}
...
export function findDistinctSegments(linestrings) {
  ...
  return featuresCoords.map((coordinates) =>
    distinctSegment(coordinates, coordCounts)
  );
}
...
function mergeRoutes(featureCollections) {
  return featureCollections.reduce((acc, feature) => {
    if (acc[feature.properties.routeIndex]) {
      acc[feature.properties.routeIndex].geometry.coordinates = [
        ...acc[feature.properties.routeIndex].geometry.coordinates,
